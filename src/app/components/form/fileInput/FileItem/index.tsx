...
const fileItem = tv({
  slots: {
    container:
      'group flex w-full items-start gap-4 rounded-lg border border-zinc-200 p-4 ',
    icon: 'rounded-full border-4 border-violet-100 bg-violet-200 p-2 text-violet-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc5300',
    deleteButton: '',
  },
  variants: {
    state: {
      progress: 'bg-violet-50',
      complete: {
    ...
    <div key={name} className={container()}>
      ...
      {state === 'error' ? (
        <div className="flex flex-1 flex-col items-start gap-1">
          ...
          <button className="text-sm font-medium text-error-700 hover:text-error-900 dark:text-error-400 dark:hover:text-error-300">
            Try again
          </button>
        </div>
      ) : (
        <div className="flex flex-1 flex-col items-start gap-1 dark:text-zinc-100">
          <div className="flex flex-col">
          ...
          <div className="flex w-full items-center gap-3">
            <div className="h-2 flex-1 rounded-full bg-zinc-600">
              <div
                className={clsx(
                  'h-2 rounded-full bg-violet-600 dark:bg-zinc-600',
